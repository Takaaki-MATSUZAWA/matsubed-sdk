...
if __name__ == '__main__':
    ...
    for test_no in p:
        ...
        # append mbed library direcroy path
        if not "mbed" in  os.listdir(project_dir_path):
            test.dependencies.append("..\library\mbed")

        # append library direcroy path
        for x in os.listdir(project_dir_path):
            if os.path.isfile(project_dir_path +"\\"+ x) and os.path.splitext(x)[-1] == '.lib':
                if not os.path.splitext(x)[0] in os.listdir(project_dir_path):
                    test.dependencies.append("..\\library\\" + os.path.splitext(x)[0])
